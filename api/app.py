...
def parse_search_parameters(request_args) -> dict:
    ...
    season = request_args.get("season")
    week_type = request_args.get("week_type") 
    time_of_day = request_args.get("time_of_day")
    
    # Default to current time if no date provided
    if date_str:
        try:
            target_datetime = pd.to_datetime(date_str).to_pydatetime()
        except Exception:
            target_datetime = datetime.now()
    else:
        target_datetime = datetime.now()
    
    # Override datetime components based on search parameters
    if season:
        target_datetime = apply_season_to_datetime(target_datetime, season)
    
    if week_type:
        target_datetime = apply_week_type_to_datetime(target_datetime, week_type)
    
    if time_of_day:
        target_datetime = apply_time_of_day_to_datetime(target_datetime, time_of_day)
...
def get_current_israeli_season() -> str:
    """Get current season in Israel."""
    now = datetime.now()
    return get_season_from_month(now.month)

def get_season_from_month(month: int) -> str:
    """Convert month to Israeli season."""
    if month in [12, 1, 2]:
        return 'winter'
    elif month in [3, 4, 5]:
        return 'spring'
...
def apply_season_to_datetime(dt: datetime, season: str) -> datetime:
    """Adjust datetime to represent the specified season."""
    season_months = {
        'winter': 1,  # January
        'spring': 4,  # April
        'summer': 7,  # July
    ...
    target_month = season_months.get(season, dt.month)
    try:
        return dt.replace(month=target_month)
    except ValueError:
        # Handle day overflow (e.g., Feb 30 -> Feb 28)
        return dt.replace(month=target_month, day=min(dt.day, 28))

def is_israeli_weekend(dt: datetime) -> bool:
    """Check if datetime falls on Israeli weekend (Thursday, Friday, Saturday)."""
    weekday = dt.weekday()  # 0=Monday, 6=Sunday
    return weekday in [3, 4, 5]  # Thursday, Friday, Saturday

def apply_week_type_to_datetime(dt: datetime, week_type: str) -> datetime:
    """Adjust datetime to represent weekday or weekend."""
    current_is_weekend = is_israeli_weekend(dt)
    target_is_weekend = (week_type == 'weekend')
    ...
    days_ahead = 0
    for i in range(1, 8):
        test_date = dt + timedelta(days=i)
        if is_israeli_weekend(test_date) == target_is_weekend:
            days_ahead = i
            break
...
def apply_time_of_day_to_datetime(dt: datetime, time_of_day: str) -> datetime:
    """Adjust datetime hour based on time of day."""
    time_hours = {
        'morning': 8,
        'afternoon': 14,
        'evening': 20,
    ...
    target_hour = time_hours.get(time_of_day, dt.hour)
...
def _proba_columns(proba, start_label: int = 1):
    ...
    if not proba:
        return {}
    cols = {f"proba_{i}": float(p) for i, p in enumerate(proba, start_label)}
    try:
...
def create_prediction_response(features_gdf, predictions, metadata, search_params=None) -> dict:
    ...
    # Add search parameters to sample prediction if provided
    if search_params and 'features' in search_params:
        sample_features.update(search_params['features'])
    ...
    response = {
        ...
        "search_parameters": search_params.get('raw_params') if search_params else None,
...
# Continue with remaining helper functions and endpoints...
...
def _graph_from_edges(edges_wgs: gpd.GeoDataFrame) -> nx.Graph:
    """Build an undirected graph where nodes are rounded endpoints."""
    if edges_wgs.empty:
        return nx.Graph()
...
def simulate():
    ...
    try:
        ...
        # Apply edits → scenario edges
        scen_edges = _apply_edits(base_edges, edits, snap_tol_m=8.0)

        # Skip complex centrality for now, use simple proxy
        cen0 = {}  # Use empty centrality for now
        cen1 = {}

        # Features aligned to FEATURE_ORDER if present
        ...
        # Predict with CatBoost if loaded, else centrality proxy
        ...
        # Join BEFORE vs AFTER by edge_id
...
def predict():
    ...
    Query Parameters:
        ...
        season (str, optional): Season override (winter, spring, summer, autumn)
        week_type (str, optional): Week type override (weekday, weekend)
        time_of_day (str, optional): Time of day override (morning, afternoon, evening, night)
    ...
    try:
        ...
        # 2. Parse search parameters (season, week_type, etc.)
        search_params = parse_search_parameters(request.args)
        target_datetime = search_params['datetime']
        
        logging.info(f"Processed search parameters: {search_params['raw_params']}")
        logging.info(f"Target datetime: {target_datetime}")
        ...
        # 4. Run feature extraction pipeline with target datetime
        ...
        # 5. Override time-based features with search parameters
        if len(features_gdf) > 0:
            ...
            logging.info(f"Applied search parameters: Hour={search_features['Hour']}, "
                        f"is_weekend={search_features['is_weekend']}, "
                        f"time_of_day={search_features['time_of_day']}")
        ...
        response = create_prediction_response(features_gdf, predictions, pipeline_metadata, search_params)
...
def predict_gpkg():
    ...
    try:
        ...
        preds_gdf['search_hour'] = search_params['features']['Hour']
        preds_gdf['search_is_weekend'] = search_params['features']['is_weekend']
        preds_gdf['search_time_of_day'] = search_params['features']['time_of_day']
        preds_gdf['search_season'] = search_params['features']['season']
        ...
        search_suffix = ""
        if search_params and 'raw_params' in search_params:
            raw_params = search_params['raw_params']
            if raw_params.get('season'):
                search_suffix += f"_{raw_params['season']}"
            if raw_params.get('week_type'):
                search_suffix += f"_{raw_params['week_type']}"
            if raw_params.get('time_of_day'):
                search_suffix += f"_{raw_params['time_of_day']}"
        
        ts = dt.datetime.now().strftime("%Y%m%d_%H%M%S")
        fname = f"predictions_{_slug(place)}{search_suffix}_{ts}.gpkg"
